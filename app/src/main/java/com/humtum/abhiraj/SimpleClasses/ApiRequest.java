...
public class ApiRequest {
    ...
    public static void Call_Api (final Context context, final String url, JSONObject jsonObject,
        ...
        if(!Variables.is_secure_info) {
            ...
            Log.d("UnSplited URl -->", url);
            Log.d("Splited Url", "is " + Arrays.toString(urlsplit));    //  To show the url

            Log.d(Variables.tag, url);

            if (jsonObject != null)

                Log.d("JsonObject IN Api--> " + urlsplit[urlsplit.length - 1], jsonObject.toString());
        }
         ...
         JsonObjectRequest jsonObjReq = new JsonObjectRequest(Request.Method.POST,
                ...
                new Response.Listener<JSONObject>() {
                    ...
                    public void onResponse(JSONObject response) {
                        ...
                        if(!Variables.is_secure_info) {
                            final String[] urlsplit = url.split("/");
                            Log.d("Splited Response " + urlsplit[urlsplit.length - 1], response.toString());
                        }
